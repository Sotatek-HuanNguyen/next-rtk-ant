...
import { CategoryType, categoriesList } from '@/constants/categoriesList';
  ...
  useEffect(() => {
    if (data) {
      if (selectedFilter.length > 0) {
        setFilteredResults(
          data.filter((component) => selectedFilter.some((filter) => filter === component.category))
        );
      } else {
  ...
  return (
    <>
      <S.FilterWrapper isOpen={isOpen}>
        <S.CheckboxGroup
          onChange={(checkedValues) => setSelectedFilter(checkedValues as CategoryType[])}
        >
